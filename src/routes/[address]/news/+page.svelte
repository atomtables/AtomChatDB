...
<div>
    ...
    <main class="">
        ...
        <div class="w-11/12 mx-auto rounded-t-[75px] min-h-[calc(100vh-80px)] bg-neutral-500/10 backdrop-blur-md" transition:slide>
            {#await data.data}
                loading...
            {:then posts}
                <div class="pt-5 flex flex-col">
                    ...
                    <!-- actual menus -->
                    <div class="mx-2 flex flex-row inset-0">
                        <!-- list -->
                        <div class="min-h-[calc(100vh-170px)] border-r-2 border-neutral-500 inset-0 w-1/2 pr-2">
                            {#if posts.length > 0 || drafts.length > 0}
                                {#each drafts as draft (draft.id)}
                                    <div class="flex flex-row items-center space-x-2 hover:bg-neutral-500/20 p-2 rounded-md cursor-pointer"
                                         onclick={() => {
                                             currentlySelected = draft.id;
                                         }}>
                                        <div class="w-10 h-10 bg-neutral-500/20 rounded-full flex items-center justify-center">
                                            <span class="text-lg font-bold">{draft.title[0]}</span>
                                        </div>
                                        <div class="flex flex-col">
                                            <span class="font-bold">{draft.title}</span>
                                            <span class="text-sm text-neutral-400">{draft.content.slice(0, 50)}...</span>
                                        </div>
                                    </div>
                                {/each}
                            {:else}
                                    ...
                                    <div class="flex flex-col">
                                        ...
                                        <div class="flex flex-row space-x-2 w-full px-2 py-2 border-b-2 border-neutral-500">
                                            ...
                                            <input
                                                ...
                                                class="border-b-1 border-neutral-500 w-full"
                                            >
                                        </div>
                                        <div class="flex flex-col w-full h-full px-2 py-2">
                                            <textarea
                                                ...
                                                class="border-b-1 border-neutral-500 w-full h-64 resize-y"
                                            ></textarea>
                                        </div>
                                    </div>
                                {/if}
